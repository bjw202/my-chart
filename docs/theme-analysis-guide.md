...
## 5-1. 다른 메뉴 갔다 와도 다시 크롤링하던 문제 (v1.0.5 amendment, 2026-05-07)

SPEC-003 ship 직후 사용자께서 발견하신 문제:

> "한 번 크롤링 했는데, 다른 메뉴 갔다오면 왜 다시 크롤링을 하느라 시간을 쓰지?"

### 무엇이 일어나고 있었나

테마 분석은 한 번 데이터를 가져오는 데 약 30초가 걸립니다. 그런데:

- 테마 분석 탭에서 결과를 확인하고
- 시장 개요 탭으로 갔다가
- 다시 테마 분석 탭으로 돌아오면

화면이 **그대로 보존**되어 있었습니다. 여기까지는 정상이었습니다.

문제는 사용자가 화면 위 **"빠른 조회 / 전체 조회"** 토글 버튼을 누르거나, \*\*페이지를 새로고침(F5)\*\*하면 또 30초를 처음부터 기다려야 한다는 점이었습니다. 데이터를 한 번 받았는데도, 컴퓨터는 매번 네이버에 새로 요청을 보내고 있었습니다.

### 왜 그렇게 되어 있었나

이 시리즈는 처음에 "**가능한 한 새로운 데이터를 받자**"는 가정으로 만들어졌습니다. 사용자가 클릭할 때마다 최신 시세를 보여주는 게 더 좋다고 본 것이죠. 캐시(저장해 두기)는 일부러 두지 않았습니다.

그러나 사용자께서는 다르게 사용하고 계셨습니다:

- 혼자 쓰는 서비스
- Chart Grid 화면에서도 **DB 업데이트는 본인이 직접 누를 때만** 하는 모델
- 즉 "데이터 신선도는 내가 직접 관리한다, 시스템이 알아서 새로 받지 마라"는 패턴

### 어떻게 고쳤나 (v1.0.5)

테마 분석도 같은 모델로 통일했습니다.

1. **자동 저장**: 한 번 받은 데이터는 브라우저 자체 저장소(localStorage)에 자동 저장됩니다. 키 이름은 `theme-analysis-cache-quick`(빠른 조회용)과 `theme-analysis-cache-full`(전체 조회용) 두 개입니다.
2. **즉시 표시**: 다음에 테마 분석 탭에 들어오거나 페이지를 새로고침해도, 저장된 데이터를 즉시 화면에 띄웁니다 — 1ms 이하. 30초 기다림 사라짐.
3. **🔄 갱신 버튼**: 툴바에 새 버튼이 추가됐습니다. **이 버튼을 누를 때만** 캐시를 무효화하고 네이버에서 새로 받아옵니다. 약 30초 소요.
...
| --- | --- |
| 처음 들어옴 | 30초 받기 → 저장 |
| 다른 메뉴 갔다 옴 | 화면 그대로 (이전부터 정상) |
| 페이지 새로고침 (F5) | **즉시 표시** (이전엔 30초 대기) |
| 빠른 ↔ 전체 토글 | 둘 다 받았다면 **즉시 전환** (이전엔 매번 30초) |
| 새 데이터 원함 | **🔄 갱신 버튼 클릭** → 30초 → 새 데이터 + 캐시 덮어쓰기 |

### 한 줄 요약

> 한 번 받으면 그대로 — 새로 받고 싶을 때만 🔄 갱신.

Chart Grid의 DB 수동 업데이트와 같은 패턴입니다. 사용자가 데이터 신선도를 직접 결정합니다. 자동 만료(TTL)는 의도적으로 두지 않았습니다.

### 디바이스/브라우저 한정 안내

localStorage는 사용 중인 브라우저의 사용자 컴퓨터에 저장됩니다. 다른 컴퓨터/다른 브라우저에서 접속하면 캐시가 없으므로 첫 로드 시 30초가 다시 걸립니다. 단독 사용자 시나리오에서는 문제가 되지 않습니다. 팀 사용/다중 디바이스가 필요해지면 별도 SPEC(SPEC-NAVER-THEME-CACHE backend 캐시)으로 backend 캐시를 도입할 수 있습니다.
...
**Q8. 30초가 또 흘러가는 듯한 느낌이 들면 어떻게 하나요?**

v1.0.5(§5-1) 적용 후에는 다음 두 경우에만 30초가 흐릅니다:

1. 처음 테마 분석 탭에 들어왔을 때 (캐시 부재)
2. **🔄 갱신** 버튼을 직접 누르신 경우

만약 그 외 상황(예: 페이지 새로고침, 다른 메뉴 갔다 옴)에서 30초 로딩이 또 보인다면:

- 브라우저 devtools → Application → Local Storage 에서 `theme-analysis-cache-full` 또는 `theme-analysis-cache-quick`이 비어있는지 확인 (브라우저 사생활 보호 모드는 localStorage 비활성화 가능성)
- 다른 브라우저/기기에서 접속하셨다면 자연스러운 동작 (디바이스별 캐시)

**Q9. 캐시 데이터를 강제로 지우고 새로 받고 싶으면?**

🔄 갱신 버튼을 누르면 됩니다. 또는 브라우저 devtools에서 `theme-analysis-cache-quick` / `theme-analysis-cache-full` 두 항목을 직접 삭제하면 다음 진입 시 새로 받습니다.

**Q10. 네이버 데이터 구조가 바뀌면 캐시도 자동 무효화되나요?**

캐시 항목 안에 `cache_version: 'v1'` 필드가 있습니다. 코드가 `'v2'`로 올라가면(향후 응답 schema 변경 시) 기존 v1 캐시는 자동으로 무시되고 새로 받습니다. 사용자께서 따로 신경 쓸 일은 없습니다.
...
| localStorage | 브라우저가 사용자 컴퓨터에 영구 저장하는 작은 데이터함 — 새로고침해도 살아있고, 같은 브라우저로 다시 접속하면 그대로 사용 가능 |
| 캐시 (cache) | 한 번 받은 데이터를 저장해 두고 다음 호출 시 재사용하는 기법 — 속도 향상 |
| TTL (Time To Live) | 캐시가 자동 만료되는 시간 — 본 시스템은 자동 만료 없음(수동 갱신 모델) |
...
Last updated: 2026-05-07 (SPEC-NAVER-THEME-003 v1.0.5 amendment ship 직후 — localStorage 캐시 + 🔄 갱신 버튼 도입)