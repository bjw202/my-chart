...
| 버전 | 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- | --- |
| 0.1.1 | 2026-08-13 | manager-spec | **plan-audit(iter-1, PASS-WITH-DEBT 0.85) 지적 수리.** D1 — AC-MBR-010의 거짓 반증 주장(`history[:8]`도 잡는다) 제거. 실측 결과 `history[:8]`의 span은 49로 원래 창(49)과 **동일**하여 span 경계로는 판별 불가였다. 대신 **창 양끝 프로즌 리터럴 앵커**를 추가해 실제로 잡도록 강화했다(실측 검증). D2 — AC-MBR-004에 §3.0 (c) 필수 절(잡는 잘못된 구현 / 항진명제가 아닌 근거) 추가 + 격자 공유 기원(F2 근접 형태) 명시 공개. D6 — AC-MBR-010 span 하한 49 → **47**(실측 바닥 48 대비 여유 1일 확보, 판별력 무손실 실증) + plan.md R4 노후화 리터럴 목록에 등재. D3 — L202 `뿐` 주장 범위를 AC-MBR-003으로 한정. D4 — O-M1 근거를 "잠재적(latent) 동일성"으로 정정. D5 — AC-MBR-008에 감사 시점 독립 교차검증값 기록(baseline 자체가 아님을 명시). M6 이월 — "'잡는 잘못된 구현' 열은 읽는 것이 아니라 **실행**한다"를 품질 게이트에 명문화. |
...
- **잡는 잘못된 구현**: **V1 전용.** V1은 CG-1(ISO 주당 1바)을 올바로 적용하므로 AC-MBR-003의 "고유 ISO 주 52"를 **통과한다.** 즉 **AC-MBR-003만으로는 V1을 잡지 못하며**, 그 사각을 메우는 것이 이 AC의 마지막-날짜 단언이다. (범위 한정 — V1은 AC-MBR-001의 세 리터럴도 **전부** 위반한다: `first=2025-08-22 ≠ 2025-08-14`, `last=2026-08-11 ≠ 2026-08-07`, `span=354 ≠ 358`. "V1을 잡는 것이 이 AC뿐"이라는 진술은 **거짓**이며, 여기서의 주장은 AC-MBR-003에 대해서만 성립한다.)
...
- **잡는 잘못된 구현**: 날짜를 `grid.history`에서 유도하지 **않는** 모든 구현. 실측 결과 **V0/V1/V2/V3 네 변형 전부 집합 동등성에 실패한다**(V★만 `True`). 각 변형의 `t = dates[-1]`로 창을 다시 계산해도 창은 항상 `grid.history` 위에서 52바를 잡으므로, 반환 집합이 격자 밖(V0)이거나 진행 중인 주를 포함하거나(V1) 개수가 다르면(V2/V3) 동등성이 깨진다.
- **항진명제가 아닌 근거 / F2 근접 형태의 명시 공개 [정직성 고지]**: 좌·우변이 **같은 `compute_weekly_grid(db)` 객체에서 파생된다.** 구현이 `history(grid, 52)`로 확정되고 나면 이 동등성은 본 SPEC이 강제하는 성질이 아니라 **선행 격자의 앵커 성질(TG-1)로부터 구조적으로 따라 나온다** — §3.0의 **F2 형태에 근접**하는 조건부 항진명제다. 위 실측이 보여주듯 "격자에서 유도하지 않은 구현"에 대해서는 실제 판별력이 있으나, "격자에서 유도한 뒤 미세하게 어긋난 구현"에 대해서는 좌우변이 함께 움직일 수 있다. **이 약점 때문에 비게이팅으로 유지한다.**
- **비게이팅 [명시]**: 이 절은 선행 `AC-SGR-007`(TG-1)이 이미 소유한 계약의 **재확인**이며, 본 SPEC이 새로 강제하는 요구사항이 아니다. 실패 시 선행 SPEC의 회귀를 의미하므로 리포트로 남기되 본 SPEC의 CI를 막지 않는다. **미충족 기준으로 읽지 않는다.** 본 SPEC의 실질 판별은 프로즌 리터럴 기반의 AC-MBR-001/002/003이 전담한다.
...
- **감사 시점 독립 교차검증값 [baseline이 아니다]**: plan-audit(iter-1) 시점에 **작업 트리 무수정 상태**에서 감사자와 작성자가 각각 독립 실행해 일치를 확인한 값이다.

  ```
  BreadthResult(date='2026-07-31', market='KOSPI',
                pct_above_sma50=24.242424242424242, pct_above_sma200=15.151515151515152,
                nh_nl_ratio=0.2, nh_nl_diff=-3, ad_ratio=1.5,
                breadth_score=0.0, total_stocks=33)
  compute_breadth_composite(...) = 33.598484848484844
  ```

  **이 값을 AC의 단언 우변으로 그대로 옮겨 적지 않는다.** 본 AC의 계약은 "**M1이 코드 변경 전에 직접 캡처한다**"이며, 위 값은 그 캡처가 감사 시점 기록과 어긋나지 않는지 확인하는 **대조용 참조**일 뿐이다. M1은 자체 캡처를 수행한 뒤 이 블록과 일치함을 확인하고, 불일치 시 원인을 규명하기 전에는 M2로 진행하지 않는다.
...
- **When** 프로즌 스냅샷으로 `weeks=52` 히스토리를 만들고 마지막 8개 날짜(`dates[-8:]`)를 취하면
- **Then** **두 단언이 동시에** 성립한다:
  - **(1) 창 양끝 앵커 [프로즌 리터럴]**: `window[0] == "2026-06-19"` **및** `window[-1] == "2026-08-07"`
  - **(2) span 경계**: `47 <= (date(window[-1]) − date(window[0])).days <= 56`
- **잡는 잘못된 구현 (실측 대조표 — 전부 이번 세션에서 측정했다)**:

  | 변형 | 창 실측 | (1) 앵커 | (2) span | 판정 |
  | --- | --- | :---: | :---: | :---: |
  | V★ 올바른 구현 | 2026-06-19 ~ 2026-08-07, 49일 | 통과 | 통과 | **통과(정상)** |
  | V0 현행 출하 — 원시 행 8개 | 2026-07-16 ~ 2026-08-11, **26일** | **실패** | **실패** | 잡는다 |
  | V1 CG-2 누락 | 2026-06-26 ~ 2026-08-11, **46일** | **실패** | **실패** | 잡는다 |
  | V2 off-by-one | 2026-06-12 ~ 2026-07-31, 49일 | **실패** | 통과 | **(1)이 잡는다** |
  | V3 `history(grid,51)` | 2026-06-19 ~ 2026-08-07, 49일 | 통과 | 통과 | **잡지 못한다 — 아래 한계 참조** |
  | 창 앞쪽을 잡는 오배선 `history[:8]` | 2025-08-14 ~ 2025-10-02, **49일** | **실패** | 통과 | **(1)이 잡는다** |
  | 창을 두 배로 넓히는 오배선 `history[-16:]` | 2026-04-24 ~ 2026-08-07, **105일** | **실패** | **실패** | 잡는다 |

- **판별 원리 / 한계 명시 [D1 수리 — 이전 판의 거짓 주장 철회]**: SPEC v0.1.0은 "상·하한 양쪽 경계를 두어 `history[:8]` 오배선도 잡는다"고 적었으나 **이는 거짓이었다.** 실측상 `history[:8]`의 span은 **49일로 올바른 창과 정확히 같다** — 정규 격자에서 연속한 8개 바는 **위치와 무관하게** 항상 약 49일을 덮으므로 span만으로는 창의 *위치*를 판별할 수 없다. 판별자는 "며칠인가"가 아니라 "**어느 8개인가**"이며, 그래서 (1) 창 양끝 프로즌 리터럴 앵커를 도입했다. 위 표는 (1)이 `history[:8]`을 실제로 실패시킴을 실측으로 보인다.
- **잡지 못하는 것 [정직 고지]**: **V3(`history(grid,51)`)는 이 AC를 통과한다.** V3의 마지막 8바는 V★와 문자 그대로 동일하기 때문이며, 이는 회피 가능한 결함이 아니라 이 AC의 관측 범위(마지막 8바)의 논리적 귀결이다. V3 판별은 AC-MBR-001(`span == 358`)이 전담한다. 또한 이 AC는 `compute_breadth_history`의 **출력**만 관측하므로, `market_service.py:139`의 슬라이싱 코드 자체를 바꾸는 변경은 **구조적으로 관측 대상이 아니다** — 위 표의 `history[:8]`/`history[-16:]` 행은 "그런 창을 만들어 넣으면 단언이 실패한다"는 검출력 증명이지, 소비자 코드 변경에 대한 게이트가 아니다.
- **항진명제가 아닌 근거**: (1)의 우변은 문자열 리터럴, (2)의 우변은 정수 리터럴이므로 좌변이 어떤 함수에서 나오든 좌우변이 함께 움직일 수 없다(F2 회피). 현행 구현(V0)은 두 단언 **모두**에 실패하므로 지금 반드시 RED다.
- **span 경계값의 근거 [D6 수리 — 여유 확보]**: 하한을 49에서 **47**로 내린다. V★ 52바 계열 안의 **8바 창 45개를 전수 측정한 결과 분포는 48–50일**(48일 2건: `window[30:38]`, `window[41:49]`)이며, 하한 49는 **여유 0**으로 스냅샷 갱신 시 올바른 코드에서 실패할 수 있었다. 47은 실측 바닥 48 대비 1일 여유를 두면서 **판별력을 전혀 잃지 않는다** — 위 표대로 V0(26)·V1(46)은 47 하한에서도 여전히 실패한다(46 이하로 내리면 V1이 span 단언을 통과하므로 46을 택하지 않았다). 상한 56(= 7×8)은 유지한다. **이 경계는 프로즌 스냅샷 파생값이므로 갱신 시 재도출 대상이다** — plan.md R4에 등재한다.
...
| # | 질문 | 왜 지금 답할 수 없는가 |
| --- | --- | --- |
| **O-M1** | `compute_breadth`의 `market` 인자가 종목을 필터하지 않는 것은 의도인가, 미구현인가? (§7.2) | 코드에도 설계 문서(`docs/sector-ux/*`)에도 근거가 없다. `BreadthResult.market`에 값을 싣는 것으로 보아 분리 의도가 있었을 수 있다. **동일성은 현재 잠재적(latent)이며 사용자에게 보이지 않는다** — 실측: `backend/schemas/market.py:34`가 `kosdaq: BreadthData \| None = None`을 선언하지만 `grep -n kosdaq backend/services/market_service.py` **0건**(한 번도 채워지지 않는다), `grep -rn "breadth.kosdaq" frontend/src/` **0건**(렌더링되지 않는다). 따라서 이는 **출하된 오계산이 아니라 미구현 필드**이며, 이 사실은 범위 제외 판정을 약화시키는 것이 아니라 **강화한다**(사용자 가시 결함이 아니므로 본 SPEC과 함께 고칠 긴급성이 없다). 의도/미구현 여부는 여전히 사용자 확인이 필요하다. |
...
- [ ] §3.0 금지 형태 F1~F4에 해당하는 단언이 신규 테스트에 **0건**
- [ ] **[HARD · plan-audit iter-1 이월] "잡는 잘못된 구현" 열은 읽지 않고 실행한다.** SPEC v0.1.0의 AC-MBR-010은 그 열에 **거짓 주장**을 담고 있었고(`history[:8]`을 잡는다 — 실측 반증), 열을 *읽는* 리뷰는 그 거짓을 통과시켰다. 따라서 M6 리뷰는 각 AC의 "잡는 잘못된 구현" 열에 이름 붙은 **모든 변형을 실제로 구성해 단언에 투입하고 실패를 관측한 축자 출력**을 progress.md에 남긴다. 열을 대조·낭독하는 것만으로는 이 항목을 충족하지 않는다.
- [ ] 변형 되돌림 대조 **실행** 기록: V0 / V1 / V2 / V3 / `history[:8]` / `history[-16:]` / AC-MBR-006 스캔 되돌림 — 각각 지정된 AC가 **실패함**을 실행으로 확인(단, AC-MBR-010 × V3는 통과가 정상 — AC-MBR-010의 "잡지 못하는 것" 고지 참조)
