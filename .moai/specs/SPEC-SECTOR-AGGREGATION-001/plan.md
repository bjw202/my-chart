...
### 0.3 v0.5.0 — AC 전수 도달가능성 스윕 결과 일괄 정정 (2026-08-13)

plan-audit iteration 3은 **FAIL 0.81 + STOP 신호**였다(iteration 2의 0.845 대비 회귀). 감사는 이 회귀를 **문서 품질 저하가 아니라 감사 심도 증가**로 귀인했고 iteration 2의 실제 점수를 `≈0.80`으로 재평가했다. 사용자 결정: **4차 감사 대신 49개 AC 전수 도달가능성 스윕 → 단일 배치 정정**.

**왜 배치인가 — 같은 뿌리의 결함이 네 번 따로 나왔다.** `cap_eff = max(0.10, 1/n)` 축퇴를 리터럴 `0.10`으로 잘못 적는 결함이 AC-SAG-002(D16, **run-phase M2 차단**) → AC-SAG-010(D22) → AC-SAG-003(D17, v0.4.2에서 수정) → AC-SAG-041(N1, 이번 스윕) 순으로 **한 번에 하나씩** 적발됐고, 매번 감사 1회 또는 run-phase 중단 1회를 소모했다. 개별 AC를 사후에 고치는 방식은 다섯 번째를 막지 못한다.

| 정정 축 | 대상 | 처분 |
| --- | --- | --- |
| **A. 축퇴 계열 구조적 종결** | INV-CAP-1 신설 + AC 11개 결속 + **AC-SAG-050**(정적 스캔 기계 집행) | acceptance.md §0 · AC-SAG-050 |
| **B. 도달 불가 임계** | 041(N1) / 010(D22) / 045 R1(D20) / 015(D23) / 045 R5-b(D21) | 순수 합성 유도 · 집합 형태 · 비게이팅 강등 |
| **C. 검출력 없는 단언** | 012(N4 항진명제) / 016(N3 산문 매치) / 044(N7 수치 목표 부재) / 013(D24 자기모순) | 변형 대조 승격 · AST 스캔 · 기대값 0 · 강등 |
| **D. 규약 충돌** | F7 NULL `MAX52`(N2 — 35 vs 15) | **규약 Y 확정** + 빌더 변경 지시 |
| **E. 공허 요건** | F4 · F8 폐지, F5-a · F6 · F10 소비 AC 재결속 | acceptance.md §8.2 |
| **F. 재빌드 계약** | R-C1~R-C8 → **F13** 6개 기계 검사 조건 | acceptance.md §8.2.1 |
| **G. 합성 바 명문화** | 라이브에 `2026-08-11` 바 부재 → 재라벨링 사실 계약화 | acceptance.md §8.1.1 · F13-6 |
| **H. 고아 AC** | 037(N5) · 049(D26) 마일스톤 결속 | plan.md M1.1 · M2 · M6 RED 목록 |

**[HARD] run-phase가 가장 먼저 읽어야 할 두 항목**:

1. **F13-1 상위집합은 협상 대상이 아니다.** 재빌드를 "라이브에서 섹터당 상위 시총 n종목 선정"으로 수행하면 **F5-a(7→0) · F5-b(10섹터→3) · F6(소멸) · F7(15→2)이 동시에 깨진다** — 네 요건이 요구하는 종목(NULL 시총 · RS 결측 · 소형 섹터 · MAX52 판정이 갈리는 종목)이 전부 시총 하위권에 몰려 있어 시총순 선정이 구조적으로 배제하기 때문이다. **현행 픽스처 145종목을 전량 보존한 위에 증분 선정**한다.
2. **F7 규약 Y는 빌더 코드 변경을 요구한다.** `build_fixture.py:481`의 `(max52 or 0.0)` / `MAX52.fillna(0.0)`을 **NULL 제외**로 교체하고 MANIFEST `f7_*`를 재기록한다. 프로덕션 `near_52w_high` 경로에도 같은 규약을 적용한다. 이 변경 없이는 AC-SAG-048의 "MANIFEST 실측 일치" 절이 **어느 값을 옳다고 할지 결정할 수 없다**.

> **plan-auditor 캐시 무효화 (v0.5.0)**: plan 산출물 4종이 전부 재변경됐으므로 plan-artifact hash가 바뀌었고 **iteration 3 판정(FAIL 0.81)도 캐시로서는 무효**다. `/moai run` Phase 1에서 plan-audit이 재실행된다(skip 4조건 중 artifact-hash 조건 불충족).
...
- **[v0.5.0 최우선 — F13-1 상위집합 [HARD]] 재빌드는 현행 픽스처(145종목)의 종목 집합을 전량 포함한다.** 신규 시총순 선정만으로 뽑으면 **F5-a(7→0) · F5-b(10섹터→3) · F6(패션 소멸) · F7(15→2)이 동시에 깨진다** — 네 요건이 요구하는 종목이 전부 시총 하위권에 몰려 있기 때문이다(실측 2026-08-13, 독립 참조 구현). 상세 계약은 acceptance.md §8.2.1 F13. **F13-2**(유효 시총 `n >= 15` 섹터 `>= 14`) · **F13-3**(F12-b `>= 9` 이고 F12-c `>= 9` — 임계 `>= 3`/`>= 5`보다 높은 빌드 목표) · **F13-4**(패션 5종목/유효 시총 3 유지, F3 소형 섹터 구성수 유지) · **F13-5**(양 시장 비공백) · **F13-6**(합성 바 재라벨링 재현 + MANIFEST `synthetic_bar` 기록)도 함께 충족한다.
- **[v0.5.0 — 빌더 변경 지시 (N2)] F7 계수 규약을 Y로 교체한다.** `build_fixture.py:481`의 `(max52 or 0.0)` / `MAX52.fillna(0.0)`은 NULL `MAX52` 종목을 `Close >= 0` = 항상 True로 처리해 divergent를 **35**로 계수한다(MANIFEST 기록값). AC-SAG-024 본문이 요구하는 규약(NULL 종목을 신·구 양쪽 분모에서 제외)으로 재계산하면 **15**다 — 차이 20은 정확히 NULL `MAX52` 종목 수다. **규약 Y로 교체하고 MANIFEST `f7_*`를 재기록**하며, 프로덕션 `near_52w_high` 경로에도 같은 규약을 적용한다.
- **[v0.4.2 신설 · v0.5.0 갱신] 빌드 목표 구성 — 섹터당 `n >= 15`**. F12-a의 하한은 `n >= 11`(관측 가능성의 원리적 경계)이지만, 실측상 `n=11`에서는 F12-b/F12-c가 임계에 붙거나 주봉 바에 따라 미달한다(F12-b `3/4/4`, F12-c `4/7/2`). `n=15`에서는 F12-b `10/10/9`, F12-c `8/10/8`로 전 바에서 여유를 갖는다. F3(KOSPI 정확히 4가 2섹터 · 정확히 5가 1섹터)·F6(유효 시총 정확히 3인 섹터)에 배정된 섹터는 **작게 유지**하고, 나머지 **12섹터 이상을 `n = 15`로** 채운다. 라이브 실측상 29개 전 섹터가 유효 시총 종목 `>= 11`을 보유하므로(헬스케어 328 · 인터넷 171 · 반도체 163 · …) 원본 데이터의 제약은 없다.
- **[v0.4.2 신설 · v0.5.0 대체] 픽스처 크기 예산** — **아래 v0.4.2 추정치는 F13-1 상위집합을 고려하지 않은 값이므로 acceptance.md §8.2.1의 갱신 예산(약 360종목 / 26섹터 / 16~17 MB)으로 대체한다.** 초과 시 F13-2의 대형 섹터 수를 14 → 12로 먼저 줄이고(F12-a 여유 1.33배 → 1.17배), **F13-1 상위집합은 마지막까지 줄이지 않는다.** (참고 — v0.4.2 원문) 현행 9.0 MB(weekly.db 6.80 MB / daily.db 2.56 MB / registry.xlsx 13 KB, 종목 145 · weekly 20,479행 · daily 9,050행). 위 구성은 종목 수를 **약 145 → 240~255**로 늘린다. 신규 종목은 F9의 완성 바 창(`2025-07-01~`, 약 58 주봉 바)에만 행을 채우므로 weekly 증분은 `+100종목 × 58행 ≈ +5,800행`(20,479 → 약 26,300행, **+28%**), daily 증분은 3M 창 약 65행 × 100종목 `≈ +6,500행`(9,050 → 약 15,500행, **+71%**). 예상 총량 **약 13 MB(+45%)**. 리포 커밋 대상으로 허용 범위이며, 초과 시 F12 무관 섹터(F3/F6 배정 섹터)를 먼저 줄인다.
...
- RED: AC-SAG-036, AC-SAG-038, AC-SAG-043(4단계 절), AC-SAG-008(지표별 커버리지), **AC-SAG-037(4단계 절 — v0.5.0 결속, N5)**
  - **[v0.5.0 신설 — N5]** AC-SAG-037은 v0.4.2까지 **어느 마일스톤 RED 목록에도 없었다**(§6 검증 순서 5번 항목에만 언급). 의존 픽스처 `fixture_max_ne_canonical`은 존재가 확인됐다(`tests/test_consumer_dates.py:57,126`, ① `completed`). M1.1에서는 7개 엔드포인트의 `as_of_date` 일치 + 정규 대표 바 동등 + `grid_version` 절을, **7개 되돌림 변형 행렬은 M6**에서 수행한다
...
- RED: AC-SAG-001 ~ AC-SAG-010, **AC-SAG-049(상한 재배분 무작위 스윕 — v0.5.0 결속, D26)**, **AC-SAG-050(INV-CAP-1 정적 스캔 1 — v0.5.0 신설)**
  - **[v0.5.0 신설 — D26]** AC-SAG-049는 v0.4.2에서 신설됐으나 **어느 마일스톤 RED 목록에도 등재되지 않았다.** 대상 모듈 `my_chart/analysis/weighting.py`가 M2 산출물이므로 M2가 정확한 결속 지점이다. AC-SAG-050의 스캔 1(코드 대상)도 같은 산출물에 의존한다
...
- RED: AC-SAG-039 ~ AC-SAG-042, **AC-SAG-037(7개 되돌림 변형 행렬 — v0.5.0 결속, N5)**, AC-SAG-007 전체 · AC-SAG-043 파생 구조 절(`deferred-to-M6`, v0.4.2 D19)
...
- **프로즌 픽스처 확인** (acceptance.md §8): 게이팅 AC가 **각각 지정된** 픽스처 위에서 실행되는지 확인한다. **v0.5.0 게이팅 열거 = 002 / 007 / 011 / 012 / 013 / 014 / 024 / 030 / 045(R1·R3·R4·R5-a·R6) / 046 / 047 / 048** — acceptance.md §8.4 규약 6과 **바이트 단위로 일치해야 한다**. v0.4.1 대비 변경: **012 승격**(N4 — 항진명제였던 mock 인자 절을 비게이팅으로 강등하고 변형 2종 대조를 주 단언으로 올렸다), **045 R5 → R5-a 단독**(R5-b는 D21로 비게이팅 강등), **013의 게이팅 지위는 파생 규칙 대조 절 + null 섹터 집합 동등 절**(부호 분산 절은 D24로 강등), **015는 게이팅 아님**(D23 — 정합성 대조가 비게이팅 스모크로 강등), **049 · 050은 게이팅 아님**(순수 합성 스윕 / 정적 스캔). (007은 D9로 순수 합성 열거에서 이동, 014는 D6으로 승격 후 D15로 픽스처·`as_of` 지정, 047은 D4로 신설, 048은 v0.4.1 D14로 신설). 픽스처 배정: **046만 날짜 축 픽스처**(`weekly-2026-08-12/`), 나머지 횡단면 AC는 **집계 픽스처**(`aggregation-2026-08-11/`). 이 열거는 acceptance.md §8 규약 6과 바이트 단위로 일치해야 한다. `/api/db/update` 1회 실행 후 재실행해 붉어지지 않음을 검증한다
- **집계 픽스처 F1~F13 충족 재확인** (acceptance.md §8.2 · §8.2.1 · **AC-SAG-048**) — v0.5.0에서 **F4 · F8이 폐지**되고 **F13(재빌드 구성 계약) 6조건이 신설**됐다. — MANIFEST 실측 기록과 실제 픽스처 내용의 일치를 확인한다. 이 검사의 **최초 실행 지점은 M1.0-a 종료**이며(v0.4.1 D14), M7에서는 회귀 확인이다
...
- **AC-SAG-050 정적 스캔 2종 확인** (v0.5.0 신설) — INV-CAP-1 작성 규약 위반이 코드·SPEC 본문 양쪽에서 0건. 대조 변형 `mut_reintroduce_cap_literal`로 스캔 2의 검출력을 실증한다
...
5. 7개 엔드포인트 계약 스캔 (AC-SAG-036 · **AC-SAG-037** — v0.5.0에서 M1.1(4단계 절) / M6(7-변형 행렬) RED 목록에 결속됐다, N5)
5-b. INV-CAP-1 정적 스캔 2종 (AC-SAG-050)
