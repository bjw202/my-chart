...
tests/fixtures/frozen/aggregation-2026-08-11/
  ...
  MANIFEST.md      ← as_of: 2026-08-11 · git SHA · 원본 DB mtime · 빌드 명령 · F2~F8 및 F12 실측 충족값
```

> **[v0.4.2 — D16 CRITICAL] 이 마일스톤은 재빌드 대상이다.** v0.4.1 요건으로 빌드된 픽스처(`adb1f25`)는 F1~F11을 전부 충족하고 AC-SAG-048도 PASS였으나, AG-5 통과 18섹터 중 `n > 10`이 **게임(32) 하나뿐**이라 §3.1의 `n <= 10` 축퇴에 걸려 **AC-SAG-002가 완전한 무게이팅**이 됐다(실측: 16섹터 중 `|시총가중 − 등가중| >= 0.5%p`가 **1개**, `n=6` 섹터들은 차이 `0.0000%p`). 요건이 효과를 함의한다는 전제가 거짓이었으므로 **F12를 신설**하고 픽스처를 다시 빌드한다. 상세는 acceptance.md §8.2 F12 블록.

- **요건은 acceptance.md §8.2 F1~F12**이며 **값이 아니라 구조**다. 어느 섹터를 몇 개 넣을지는 빌드 재량이고, 충족 여부만 MANIFEST에 실측으로 기록한다.
- **[v0.4.2 신설] 빌드 목표 구성 — 섹터당 `n >= 15`**. F12-a의 하한은 `n >= 11`(관측 가능성의 원리적 경계)이지만, 실측상 `n=11`에서는 F12-b/F12-c가 임계에 붙거나 주봉 바에 따라 미달한다(F12-b `3/4/4`, F12-c `4/7/2`). `n=15`에서는 F12-b `10/10/9`, F12-c `8/10/8`로 전 바에서 여유를 갖는다. F3(KOSPI 정확히 4가 2섹터 · 정확히 5가 1섹터)·F6(유효 시총 정확히 3인 섹터)에 배정된 섹터는 **작게 유지**하고, 나머지 **12섹터 이상을 `n = 15`로** 채운다. 라이브 실측상 29개 전 섹터가 유효 시총 종목 `>= 11`을 보유하므로(헬스케어 328 · 인터넷 171 · 반도체 163 · …) 원본 데이터의 제약은 없다.
- **[v0.4.2 신설] 픽스처 크기 예산**: 현행 9.0 MB(weekly.db 6.80 MB / daily.db 2.56 MB / registry.xlsx 13 KB, 종목 145 · weekly 20,479행 · daily 9,050행). 위 구성은 종목 수를 **약 145 → 240~255**로 늘린다. 신규 종목은 F9의 완성 바 창(`2025-07-01~`, 약 58 주봉 바)에만 행을 채우므로 weekly 증분은 `+100종목 × 58행 ≈ +5,800행`(20,479 → 약 26,300행, **+28%**), daily 증분은 3M 창 약 65행 × 100종목 `≈ +6,500행`(9,050 → 약 15,500행, **+71%**). 예상 총량 **약 13 MB(+45%)**. 리포 커밋 대상으로 허용 범위이며, 초과 시 F12 무관 섹터(F3/F6 배정 섹터)를 먼저 줄인다.
- **[v0.4.2 신설] 재빌드는 골든 baseline 재캡처를 동반한다.** 구 baseline(`b839cee` 캡처분, `tests/fixtures/golden/pre-sector-ux/`)은 F12 미충족 픽스처 위에서 떠진 것이므로 **폐기하고 M1.0-b를 다시 수행**한다. 재캡처 사유와 새 실측값을 커밋 메시지에 남긴다(acceptance.md §8.4 규약 4). 아직 M2에 진입하지 않았으므로(구현 코드 미변경) 이 경로가 열려 있다 — acceptance.md §8.5.
...
- **[v0.4.1 (D14)] 요건 충족 검사는 M1.0-a의 종료 조건이다 — `AC-SAG-048 PASS`.** v0.4.0은 이를 "M1 종료 시"로 적어 캡처(M1.0-b) **뒤**에 두었는데, 그 순서에서는 요건 미충족 픽스처 위에서 baseline이 떠진다(iteration 1 D1 실패 모드의 한 단계 이동). AC-SAG-048은 F1~F12를 각각 독립 단언하고(F12는 v0.4.2 D16 신설), **MANIFEST에 기록된 값과 검사가 산출한 실측값의 정확한 일치**까지 요구한다(F3은 섹터명 집합까지) — AC-SAG-007 / 045 R6이 섹터명을 MANIFEST에서 읽으므로 MANIFEST가 틀리면 그 두 AC가 틀린 기대값 위에서 GREEN이 된다.
...
> **[HARD · v0.4.1] 이 마일스톤이 point of no return이다.** M2가 구 집계 구현을 교체하는 순간 골든 baseline의 재캡처가 불가능해진다. **진입 전제**: `AC-SAG-048 PASS`(집계 픽스처 F1~F12, v0.4.2 F12 포함) **및** `AC-SAG-047 PASS`(**F12 충족 픽스처 위에서 재캡처된** 골든 baseline 완결성). 둘 중 하나라도 RED면 M2에 진입하지 않는다 — M1.0 구간에서는 아직 되돌릴 수 있다.
...
- **집계 픽스처 F1~F12 충족 재확인** (acceptance.md §8.2 · **AC-SAG-048**) — MANIFEST 실측 기록과 실제 픽스처 내용의 일치를 확인한다. 이 검사의 **최초 실행 지점은 M1.0-a 종료**이며(v0.4.1 D14), M7에서는 회귀 확인이다
...
> **[v0.4.2 교체 — D17 CRITICAL] 이전 판의 알고리즘은 종료하지 않는다.** v0.4.1 verbatim 형태는 상한에 걸린 종목을 `cap_eff`로 고정하지만, **다음 반복에서 그 종목이 `over`에 속하지 않으므로**(`> cap_eff + 1e-12`가 아니다) `rest`로 분류되어 **재배분을 다시 받는다**. 그러면 다시 상한을 넘고, 이 진동이 20회 안전 상한까지 계속된 뒤 **상한을 초과한 가중치를 그대로 반환**한다. 실측(2026-08-13, seed `20260813`, 4,000 케이스, `n ∈ [2,40]`): **3,183건(79.6%)에서 상한 초과 상태로 종료**, 최악은 `n=6`에서 `cap_eff=0.166667` 대비 `max(w)=0.211925`(**+27.2%**). AC-SAG-001의 *"최대값 <= cap_eff"* 와 *"5회 이하 수렴"* 을 **동시에** 위반한다. 아래 **동결형(freeze)** 으로 교체한다.
...
def capped_weights(caps: dict[str, float], cap: float = 0.10) -> dict[str, float]:
    ...
    frozen: set[str] = set()                       # 이미 상한에 걸려 고정된 종목
    for _ in range(min(n, 20)):                    # 매 회 frozen 이 최소 1개 늘어나므로 반드시 종료
        over = {k for k, v in w.items()
                if k not in frozen and v > cap_eff + 1e-12}
        if not over:
            break
        excess = sum(w[k] - cap_eff for k in over)
        for k in over:
            w[k] = cap_eff
        frozen |= over                             # ← 핵심: 재배분 집합에서 영구 제외
...
**종료 증명**: `over`는 항상 `frozen`과 서로소이고 비어 있지 않을 때만 루프가 계속되므로, 매 반복마다 `frozen`이 **진부분집합으로 엄격히 증가**한다. `|frozen| <= n`이므로 반복은 `<= min(n, 20)`회에 종료하며, 종료 시 비동결 종목 중 상한 초과가 없고 동결 종목은 정확히 `cap_eff`이므로 `max(w) <= cap_eff + 1e-12`가 **종료 시점에** 성립한다. v0.4.1 형태와 달리 반복 상한 소진이 정상 종료 경로가 아니다.

**고정점 불변 (교체의 안전성)**: 동결형은 v0.4.1 verbatim 형태의 **고정점을 바꾸지 않는다** — verbatim 형태를 2,000회까지 돌리면 수렴하며(4,000 케이스 전부 수렴), 동결형 결과와의 최대 편차는 **`6.696e-12`**다. 독립 검증으로 닫힌 해(원비중 내림차순으로 상한 집합 `S`를 확정한 뒤 `w[k∈S] = cap_eff`, `w[k∉S] = r[k] × (1 − |S|×cap_eff) / Σ_{j∉S} r[j]`)와도 최대 편차 **`3.053e-16`**로 일치한다. 즉 이 교체는 **결과값이 아니라 종료 성질만** 고친다. 기계적 검증은 **AC-SAG-049**(시드 고정 무작위 스윕 + 대조 변형 `mut_plan31_verbatim`).

**반복 횟수 실측**: 본 스윕 최악 **6회**, 확장 스윕(60,000 케이스 · `n <= 60` · seed 7) 최악 **7회**. 따라서 §0.2 성능표와 AC-SAG-001의 *"섹터당 <= 5회"* 는 **거짓이며 폐기**한다 — 계약은 횟수 임계가 아니라 위 종료 증명(`<= min(n, 20)`)이다.

**`n <= 10`의 축퇴 [HARD · D16과 직결]**: `cap = 0.10`에서 `n <= 10`이면 `cap_eff = max(0.10, 1/n) = 1/n`이므로 `n × cap_eff = 1`이고, `max(w) <= cap_eff ∧ Σw = 1`의 해는 **균등 하나뿐**이다. 즉 **구성종목 10개 이하 섹터에서 시총가중은 등가중과 완전히 동일**하며(부동소수 오차조차 0), 상한 재배분이 관측 가능한 하한은 `n >= 11`이다. `rest_total == 0`(전 종목 동결)은 이 경우에만 발생하고 그때는 이미 균등이므로 위 코드의 `if rest_total > 0` 가드로 충분하다. 이 축퇴가 **acceptance.md §8.2 F12 신설의 직접 근거**이며, 픽스처가 이를 만족하지 못하면 AC-SAG-002가 무게이팅이 된다.
...
| **집계 픽스처 빌드가 F1~F12를 부분 충족한 채 진행** | **HIGH** | M1.0-a → M1.0-b 순서 강제 + MANIFEST에 요건별 실측 기록 + M1 종료 구조 검사. 미충족 요건에 걸린 AC는 GREEN이 아니라 **Gaps**로 기재 |
| **구조 요건이 검출력을 함의한다고 가정** (v0.4.2 — D16으로 실현됨) | **HIGH** | F4가 F12를 함의하지 않아 AC-SAG-002가 무게이팅이 된 실제 사례. 대응: 검출력에 직결된 요건은 **구조가 아니라 효과 자체를 규정**한다(F12-b / F12-c). 신규 게이팅 AC 추가 시 acceptance.md §8.4 규약 10(대조 검출력 실측)을 적용 |
