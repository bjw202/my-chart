...
- **Then** **모든** 응답이 `as_of_date`, `as_of_is_partial_week`, `market_filter`, `weight_cap`, `grid_version`, `benchmark`, `data`, `excluded`, `warnings` 9개 키를 갖는다(엔드포인트별 루프 단언).
...
6. **열거 완결성 규칙**: `AC-SAG-001`~`045` 전부가 (게이팅 표) 또는 (§8.5 순수 합성 열거) 중 **정확히 한 곳**에 나타난다. 새 AC를 추가하면 둘 중 하나에 반드시 등재한다 — 어느 쪽에도 없는 AC는 규약 밖에 방치되며, 그것이 AC-SAG-011/044가 누락된 경로였다.

---

## 9. 품질 게이트 (Definition of Done)

- [ ] AC-SAG-001 ~ AC-SAG-045 전부 PASS
- [ ] **선결 미결 해소 — 구현 착수 전 사용자 결정 필요**: **O-A8**(미완성 주 바와 기간 계산의 정합, ①의 O-G2 인수분 — **M3 차단**), **O-A7**(최소 구성수 5의 Bump 적용 — ③의 AC-SUX-019 / AC-SUX-056 R5가 의존, **③ 착수 전 필요**)
- [ ] **해결 완료 확인**: O-A1(롤링 정규화 미적용), O-A3(warnings[] + daily 최신 Close), O-A4(기간 연동 창), O-A6(지표별 coverage + 최상위 최소값) 4건이 REQ 본문에 반영됨
